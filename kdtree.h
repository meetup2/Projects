...
class KDTree
{
  ...
  private:
     ...
     */
    KDTreeNode* chelper(vector<Point<Dim>>& points, unsigned start, unsigned end, int dim);
    void quick_select(vector<Point<Dim>>& list, unsigned start, unsigned end, unsigned k, int dim);
    unsigned partition(vector<Point<Dim>>& list, unsigned start, unsigned end, unsigned pivotIndex, int dim);
    KDTreeNode* copy(const KDTreeNode* subroot);
    void free(KDTreeNode* subroot);
    Point<Dim> findNearestNeighborHelper(const Point<Dim>& query, KDTreeNode* subroot, int dim) const;
};
